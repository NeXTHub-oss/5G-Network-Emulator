...
//--------------------------------------------------------------------------------------------------
// random_walk_model(): is an overriding class which implementes a random walk mobility model. The model
// is simple, randomly selecting a new completely random direction every time the elapsed time is greater than 
// the target time.
// Input: 
...
//--------------------------------------------------------------------------------------------------
class random_walk_model : public mobility_model_base
{
public:
    random_walk_model(int id, pos2d _init_pos, bool _random_init, float _speed, float _speed_var, 
...
public:
    void update_pos(float current_t)
    {
        if(current_t >= t_target)
        {
            ...
            current_dir = {uniform_dist(uniform_gen), uniform_dist(uniform_gen)};
