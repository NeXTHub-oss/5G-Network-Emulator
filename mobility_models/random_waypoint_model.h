...
#ifndef RANDOM_WAYPOINT_MODEL_H
#define RANDOM_WAYPOINT_MODEL_H
...
//--------------------------------------------------------------------------------------------------
// random_waypoint_model(): is an overriding class which implementes a random waypoint mobility model. The model
// is simple, randomly selecting a new target position every time the previous target
...
//--------------------------------------------------------------------------------------------------
class random_waypoint_model : public mobility_model_base
{
public:
    random_waypoint_model(int id, pos2d _init_pos, bool _random_init, float _speed, float _speed_var,
...
public:
    void update_pos(float current_t)
    {
        if(current_t >= t_target)
        {
            ...
            float radius = abs(uniform_dist(uniform_gen)*max_distance);
            float theta = abs(uniform_dist(uniform_gen)*2*M_PI);
            current_dir = pos2d(radius*(float)cos(theta), radius*(float)sin(theta));
            current_dir /= current_dir.get_distance();
        }
        if(past_t > 0)
        {
            float t_step = current_t - past_t; pos2d next_pos = current_pos + current_dir * ( t_step * speed );
